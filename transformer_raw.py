...
class TimesFormer_2input(nn.Module):
    def __init__(self, configs):
        super().__init__()
        self.configs = configs
        self.TimesFormer = SpaceTimeTransformer(configs)
        self.conv1 = nn.Conv2d(self.configs.input_dim2, 16, 3, stride=1, padding=1)
        ...
        self.projection1 = nn.Linear(2, 64)
        self.projection2 = nn.Linear(64, 1)
        
    def forward(self, inputs1, inputs2, tgt, train=True):
        out1 = self.TimesFormer(inputs1, tgt, train=train) # B,S,C,H,W
        
        out2 = self.conv1(inputs2) # B,C,H,W
