...
sys.path.append(r"D:\transformer")
#sys.path.pop( )
import os
...
def read_samples0805(rootdir):   
    #读各X基本信息
    rows,cols,geotransform,projection,nDV_dem=Readxy('d://NORM_I//DEM_I//m_SRTM_GTOPO_u30_mosaic.tif') #-9999
    rows,cols,geotransform,projection,nDV_hwsd=Readxy('d://NORM_I//HWSD_I//m_T_CLAY.tif') #0
    rows,cols,geotransform,projection,nDV_imerg=Readxy('d://NORM_I//IV_USA_I//m_m_st4.2018010112.24h.tif') #-9999
    rows,cols,geotransform,projection,nDV_luc=Readxy('d://NORM_I//MCD12C1_I//m_lc_2018_o.tif') #0
    rows,cols,geotransform,projection,nDV_lst=Readxy('d://NORM_I//MCD11A1_ST//MCD11A1_ST_2018001.tif') #-9999
    rows,cols,geotransform,projection,nDV_opt=Readxy('d://NORM_I//MCD43A4_USA_ST//band1//MCD43A4_2018001.tif') #32767  
    
    file_demo = [[0.0]*cols]*rows 
    file_demo = np.array(file_demo)
    file_demo = Read('d://DEMO//01.tif')
    ...
    datarange = np.array([1508,1520,1564,1569,1596,1601,1708,1713])
    for datarange0 in range(0,len(datarange)//2):
        for date in range(datarange[datarange0*2],datarange[datarange0*2+1]): #经步长32测试发现第一个样本从20180114开始
    # for date in range(1048,1049): #经步长32测试发现第一个样本从20180114开始
            sample_s = []
            # sample_s_1 = []
            # sample_s_2 = []
            # sample_s_3 = []
            ...
            file_lst_1 = Read(rootdir_lst+'//MCD11A1_ST_'+str(lst_date[date])+'.tif')
            file_imerg_1 = [[0.0]*cols]*rows #栅格值和，二维数组
            file_imerg_1 = np.array(file_imerg_1)
            file_imerg_1 = Read(rootdir_imerg+'//m_m_st4.'+str(imerg_date[date])+'12.24h.tif')
            ...
            file_lst_2 = Read(rootdir_lst+'//MCD11A1_ST_'+str(lst_date[date-1])+'.tif')
            file_imerg_2 = [[0.0]*cols]*rows #栅格值和，二维数组
            file_imerg_2 = np.array(file_imerg_2)
            file_imerg_2 = Read(rootdir_imerg+'//m_m_st4.'+str(imerg_date[date-1])+'12.24h.tif')
            ...
            file_lst_3 = Read(rootdir_lst+'//MCD11A1_ST_'+str(lst_date[date-2])+'.tif')
            file_imerg_3 = [[0.0]*cols]*rows #栅格值和，二维数组
            file_imerg_3 = np.array(file_imerg_3)
            file_imerg_3 = Read(rootdir_imerg+'//m_m_st4.'+str(imerg_date[date-2])+'12.24h.tif')
            ...
            file_lst_4 = Read(rootdir_lst+'//MCD11A1_ST_'+str(lst_date[date-3])+'.tif')
            file_imerg_4 = [[0.0]*cols]*rows #栅格值和，二维数组
            file_imerg_4 = np.array(file_imerg_4)
            file_imerg_4 = Read(rootdir_imerg+'//m_m_st4.'+str(imerg_date[date-3])+'12.24h.tif')
            ...
            file_lst_5 = Read(rootdir_lst+'//MCD11A1_ST_'+str(lst_date[date-4])+'.tif')
            file_imerg_5 = [[0.0]*cols]*rows #栅格值和，二维数组
            file_imerg_5 = np.array(file_imerg_5)
            file_imerg_5 = Read(rootdir_imerg+'//m_m_st4.'+str(imerg_date[date-4])+'12.24h.tif')
            ...
            testmodel.to(device)
            model_info = torch.load(r'D:\DL\checkpoint_best.pth.tar')#改这里,加载最后一个checkpoint文件
            ...
            # device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
            # testmodel.to(device)
            # model_info = torch.load(r'j:\para-20240117\08-5\checkpoint_best.pth.tar')#改这里,加载最后一个checkpoint文件
            # testmodel.load_state_dict(model_info['state_dict'])
            # testset_1=ImageDataset_array_test(test_x1_1,test_x2_1)
            ...
            path = 'd://NORM_I//tf_downscale//SM_'+str(imerg_date[date])+'_0.tif'
            WriteGTiffFile(path,rows,cols,output,geotransform,projection, -9999.0,gdalconst.GDT_Float32)  
            print (path)

if __name__ == "__main__":   
    print('start from here')
    root_dir = 'd://NORM_I' #sample_5
